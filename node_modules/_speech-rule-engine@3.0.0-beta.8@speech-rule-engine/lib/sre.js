...
sre.Debugger.prototype.output_=function(a){this.outputFunction_.apply(console.info===this.outputFunction_?console:this.outputFunction_,["Speech Rule Engine Debugger:"].concat(a))};sre.Debugger.prototype.output=function(a){this.isActive_&&this.output_(Array.prototype.slice.call(arguments,0))};sre.Debugger.prototype.generateOutput=function(a){this.isActive_&&this.output_(a.apply(a,[]))};sre.Debugger.prototype.exit=function(a){this.isActive_&&this.stream_&&this.stream_.end("","",a||function(){})};sre.EnrichCase=function(){};sre.EnrichCase.prototype.getMathml=function(){};sre.AbstractEnrichCase=function(a){this.semantic=a};sre.AbstractEnrichCase.prototype.getMathml=goog.abstractMethod;sre.EnrichCaseFactory=function(){};sre.EnrichCaseFactory.cases=[];sre.EnrichCaseFactory.getCase=function(a){for(var b=0,c;c=sre.EnrichCaseFactory.cases[b];b++)if(c.test(a))return new c.constr(a);return null};sre.SemanticUtil=function(){};sre.SemanticUtil.objectsToKeys=function(a){a=Array.prototype.slice.call(arguments,0);var b=[];return b.concat.apply(b,a.map(Object.keys))};sre.SemanticUtil.objectsToValues=function(a){a=Array.prototype.slice.call(arguments,0);var b=[];a.forEach(function(a){for(var c in a)b.push(a[c])});return b};
...
c);return c}sre.Debugger.getInstance().generateOutput(goog.bind(function(){return["Apply Rule:",d.name,d.dynamicCstr.toString(),b.mode!==sre.Engine.Mode.HTTP?a.toString():a]},this));var e=d.context||this.activeStore_.context,f=d.action.components;c=[];for(var g=0,h;h=f[g];g++){var k=[],l=h.content||"",m=h.attributes||{},n=!1;h.grammar&&this.processGrammar(e,a,h.grammar);var p=null,r=null;if(m.engine){p=sre.Engine.getInstance().dynamicCstr.getComponents();var t=sre.Grammar.parseInput(m.engine);r=this.cache_;
this.clearCache();sre.Engine.getInstance().setDynamicCstr(t)}switch(h.type){case sre.SpeechRule.Type.NODE:(l=e.applyQuery(a,l))&&(k=this.evaluateTree_(l));break;case sre.SpeechRule.Type.MULTI:n=!0;l=e.applySelector(a,l);0<l.length&&(k=this.evaluateNodeList_(e,l,m.sepFunc,e.constructString(a,m.separator),m.ctxtFunc,e.constructString(a,m.context)));break;case sre.SpeechRule.Type.TEXT:(l=e.constructString(a,l))&&(k=[sre.AuditoryDescription.create({text:l},{adjust:!0})]);break;default:k=[sre.AuditoryDescription.create({text:l})]}k[0]&&
!n&&(m.context&&(k[0].context=e.constructString(a,m.context)+(k[0].context||"")),m.annotation&&(k[0].annotation=m.annotation));h.grammar&&sre.Grammar.getInstance().popState();c=c.concat(this.addPersonality_(k,m,n,a));p&&(this.cache_=r,sre.Engine.getInstance().setDynamicCstr(p))}this.pushCache_(a,c);return c};
...
sre.System.prototype.walk=function(a){return sre.ProcessorFactory.output("walker",a)};sre.System.prototype.move=function(a){return sre.ProcessorFactory.keypress("move",a)};sre.Api={};module.exports.version=sre.System.getInstance().version;module.exports.toSpeech=sre.System.getInstance().toSpeech;module.exports.toSemantic=sre.System.getInstance().toSemantic;module.exports.toJson=sre.System.getInstance().toJson;module.exports.toDescription=sre.System.getInstance().toDescription;module.exports.toEnriched=sre.System.getInstance().toEnriched;module.exports.walk=sre.System.getInstance().walk;module.exports.move=sre.System.getInstance().move;module.exports.file={};
module.exports.file.toSpeech=sre.System.getInstance().fileToSpeech;module.exports.file.toSemantic=sre.System.getInstance().fileToSemantic;module.exports.file.toJson=sre.System.getInstance().fileToJson;module.exports.file.toDescription=sre.System.getInstance().fileToDescription;module.exports.file.toEnriched=sre.System.getInstance().fileToEnriched;module.exports.setupEngine=sre.System.getInstance().setupEngine;module.exports.pprintXML=sre.DomUtil.formatXml;module.exports.engineReady=sre.System.getInstance().engineReady;
module.exports.engineSetup=sre.System.getInstance().engineSetup;global&&global.SRE_JSON_PATH&&sre.System.setAsync();sre.Cli=function(){this.system=sre.System.getInstance();this.setup={mode:sre.Engine.Mode.SYNC};this.processors=[];this.dp=new sre.SystemExternal.xmldom.DOMParser({errorHandler:function(a,b){throw new sre.Engine.Error("XML DOM error!");}})};sre.Cli.prototype.set=function(a,b){this.setup[b]="undefined"===typeof a?"semantics"===b?!1:!0:a};sre.Cli.prototype.processor=function(a,b){this.processors.push(b)};
sre.Cli.prototype.enumerate=function(){this.system.setupEngine(this.setup);var a=sre.DynamicCstr.DEFAULT_ORDER.map(function(a){return a.length}),b=function(b,c){a[c]=Math.max.apply(null,Object.keys(b).map(function(a){return a.length}).concat(a[c]))},c=function(a,b){return a+Array(b-a.length+1).join(" ")},d=sre.SpeechRuleEngine.getInstance().enumerate();d=sre.MathCompoundStore.getInstance().enumerate(d);var e=[];b(d,0);for(var f in d){var g=!0,h=d[f];b(h,1);for(var k in h){var l=!0,m=h[k];b(m,2);for(var n in m){var p=
Object.keys(m[n]).sort();if("clearspeak"===n){p=!0;var r=sre.ClearspeakPreferences.getLocalePreferences(d)[f],t;for(t in r)e.push([c(g?f:"",a[0]),c(l?k:"",a[1]),c(p?n:"",a[2]),r[t].join(", ")]),p=!1}else e.push([c(g?f:"",a[0]),c(l?k:"",a[1]),c(n,a[2]),p.join(", ")]);l=g=!1}}}var u=0,q="";q+=sre.DynamicCstr.DEFAULT_ORDER.slice(0,-1).map(function(b){return c(b,a[u++])}).join(" | ");q+="\n";a.forEach(function(a){q+=Array(a+3).join("=")});q+="========================\n";q+=e.map(function(a){return a.join(" | ")}).join("\n");
console.info(q)};sre.Cli.prototype.execute=function(a){var b=sre.SystemExternal.commander;this.runProcessors_(goog.bind(function(a,d){this.system.processFile(a,d,b.output)},this),a)};sre.Cli.prototype.runProcessors_=function(a,b){try{this.processors.length||this.processors.push("speech"),b&&this.processors.forEach(function(c){a(c,b)})}catch(c){console.error(c.name+": "+c.message),sre.Debugger.getInstance().exit(function(){sre.SystemExternal.process.exit(1)})}};
sre.Cli.prototype.readline=function(){var a=sre.SystemExternal.commander;sre.SystemExternal.process.stdin.setEncoding("utf8");var b=sre.SystemExternal.require("readline").createInterface({input:sre.SystemExternal.process.stdin,output:a.output?sre.SystemExternal.fs.createWriteStream(a.output):sre.SystemExternal.process.stdout}),c="";b.on("line",goog.bind(function(a){c+=a;this.readExpression_(c)&&b.close()},this));b.on("close",goog.bind(function(){this.runProcessors_(goog.bind(function(a,c){b.output.write(sre.ProcessorFactory.output(a,
c)+"\n")},this),c)},this))};sre.Cli.prototype.readExpression_=function(a){try{this.dp.parseFromString(a,"text/xml")}catch(b){return!1}return!0};
sre.Cli.prototype.commandLine=function(){var a=sre.SystemExternal.commander,b=this.system,c=goog.bind(this.set,this),d=goog.bind(this.processor,this);a.version(b.version).usage("[options] <file ...>").option("").option("-i, --input [name]","Input file [name]. (Deprecated)").option("-o, --output [name]","Output file [name]. Defaults to stdout.").option("").option("-d, --dom [name]","Domain or subject area [name].",c,"domain").option("-t, --style [name]","Speech style [name].",c,"style").option("-c, --locale [code]",
"Locale [code].",c,"locale").option("-b, --modality [name]","Modality [name].",c,"modality").option("-s, --semantics","Switch OFF semantics interpretation. (Deprecated)",c,"semantics").option("-k, --markup [name]","Generate speech output with markup tags.",c,"markup").option("-r, --rate [value]","Base rate [value] for tagged speech output.",c,"rate").option("").option("-p, --speech","Generate speech output (default).",d,"speech").option("-a, --audit","Generate auditory descriptions (JSON format).",
d,"description").option("-j, --json","Generate JSON of semantic tree.",d,"json").option("-x, --xml","Generate XML of semantic tree.",d,"semantic").option("").option("-m, --mathml","Generate enriched MathML.",d,"enriched").option("-g, --generate <depth>","Include generated speech in enriched MathML (with -m option only).",c,"speech").option("-r, --structure","Include structure attribute in enriched MathML (with -m option only).",c,"structure").option("").option("-P, --pprint","Pretty print output whenever possible.",
c,"pprint").option("-v, --verbose","Verbose mode.").option("-l, --log [name]","Log file [name].").option("--options","List engine setup options.").on("option:options",goog.bind(function(){this.enumerate();sre.SystemExternal.process.exit(0)},this)).parse(sre.SystemExternal.process.argv);this.system.setupEngine(this.setup);a.verbose&&sre.Debugger.getInstance().init(a.log);a.input&&this.execute(a.input);a.args.length?a.args.forEach(goog.bind(this.execute,this)):this.readline();sre.Debugger.getInstance().exit(function(){sre.SystemExternal.process.exit(0)})};
sre.SystemExternal.process&&sre.SystemExternal.process.env.SRE_TOP_PATH&&(new sre.Cli).commandLine();
